"""Post a message to a named team channel / webhook.

Standalone CLI usage::

    python -m nerya.skills.builtin.notify.scripts.post_channel \\
        --json '{"channel": "alerts", "text": "Drawdown breach on demo-strategy"}'

The ``channel`` value should match an entry in
``messages/channels.yml`` — the message pipeline resolves it to the
underlying transport (Slack webhook, Discord webhook, Telegram chat
id, etc.). If the channel is unknown the message still lands in the
outbox + journal, so the operator can see why nothing went through.

Output schema::

    {"ok": true, "message_id": str, "state": "queued", "channel": str}
"""
...
def main() -> None:
    ...
    channel = args.channel or payload.get("channel")
