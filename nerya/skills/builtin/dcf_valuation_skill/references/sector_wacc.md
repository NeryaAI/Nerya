...
Adapted from `dexter` (MIT). Use these typical WACC ranges as starting
points, then adjust based on company-specific factors.

## Determining Company Sector

Use the `equity_research` skill's `fetch_financials.py` with
`statements: ["company_facts"]` to retrieve the company's `sector`.
