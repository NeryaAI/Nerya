...
The dispatcher already handles bounded parallelism, shared budget, and
the live-trading denylist; your job is to pick the roles, write a
single mission line, and consume the aggregated finding.
...
- **Picking 7 roles when 3 would do.** Each extra role costs USD and
...
- **Forgetting to set `usd_budget`.** Without it a runaway team can
  burn the workspace budget; always set it for low-frequency
  strategy decisions.
