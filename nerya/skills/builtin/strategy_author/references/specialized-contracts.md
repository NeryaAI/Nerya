...
Use StrategyAgentTask.dispatch/skip/error for Agent flows. Never pass context=, never wrap it in ctx.result.agent_task, and use a small object session_key, not a string. ctx.session_key does not exist and session routing is not durable deduplication; use ctx.state. Keep low-level trading action names out of user-facing docs and execution briefs; profile allowed_tools is the machine policy, not the operator explanation.
